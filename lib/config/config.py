...
class CfgNode(_CfgNode):
    ...
    def load_yaml_with_base(filename: str, allow_unsafe: bool = False):
        ...
        Just like `yaml.load(open(filename))`, but inherit attributes from its
            `_BASE_`.
        Args:
            filename (str): the file name of the current config. Will be used
                to find the base config file.
        ...
        with open(filename, "r") as f:
            try:
                ...
                logger.warning(
                    "Loading config {} with yaml.unsafe_load. Your machine "
                    "may be at risk if the file contains malicious "
